...
* 이러한 가상 머신은 하이퍼바이저, 즉 호스트 PC의 자원을 적절하게 배분받은 후 사용합니다.

#### 클라우드 컴퓨팅

* 클라우드 컴퓨팅은 위에서 말했다시피 네트워크 전체에서 컴퓨팅, 네트워크, 스토리지 인프라 자원, 서비스, 플랫폼, 애플리케이션을 <span class="evidence">사용자에게 제공하는 접근 방식입니다.</span>
* 방법론으로 다양한 용도의 다양한 자원을 사용자 그룹에게 제공합니다.
* 마지막으로 정의하자면 __가상화는 하드웨어에서 기능을 분리하는 기술__ 이고 __클라우드 컴퓨팅은 이러한 분하을 사용하는 솔루션보다 큰 개념인 방법론이다.__
...
* 서버를 관리를 직접적으로 해야 합니다. (트래픽, 프로지버닝 등등)
...
* 우리가 자주 사용하는 가상 호스팅(VM Hosting)과 비슷하나 처음에 말했다시피, 가상 호스팅은 우리가 직접 장비를 사서 그 장비의 한에서 자원을 할당하고 구성해야 하지만 __(슈퍼바이저 내에서)__, IaaS는 기업이 준비해놓은 환경에서 우리가 선택할 수 있다는 점에서 차이가 있습니다. __(기업, 클라우드가 준비된 환경도 정해져 있겠지만 방법론적으로 클라우드에서 서비스 한다는 개념)__
