...
* `sudo apt-get install ftpd ssh wireshark iperf speedometer mpv git strongswan vim net-tools g++ cmake nvidia-375 ubuntu-restricted-extras`


> 각종 프로그램 설명입니다. 

1. ```ftpd``` ftp 서버 사용을 위한 패키지입니다.
2. ```ssh``` 원격 접속을 위한 패키지입니다.
3. ```wireshark``` 패킷 캡쳐를 위한 패키지입니다.
4. ```iperf``` 네트워크 성능 측정을 위한 패키지입니다.
5. ```speedometer``` 네트워크의 대역폭에 대한 모니터링을 할 수 있는 패키지입니다.
6. ```mpv``` 리눅스에서 사용 되는 플레이어 패키지입니다.
7. ```git``` git 사용을 위한 패키지입니다.
8. ```strongswan``` IPSec 사용을 위한 패키지입니다.
9. ```vim``` vi의 업그레이드 버젼인 편집 툴 패키지입니다.
10. ```net-tools``` 네트워크 관리자가 흔히 사용하는 여러 가지 기능들(명령어, 유틸리티등)을 사용하기 쉽게 모아놓은 패키지입니다.
11. `g++` g++ 컴파일러입니다.
