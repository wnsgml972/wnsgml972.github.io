---
layout: post
title: "오픈 소스 개발에 강력한 Windows 10 Bash로 Ubuntu 개발환경 구성하기"
subtitle: "Windows Subsystem for Linux"
date: 2019-05-07
author: KimJunHee
category: Setting
tags: windows-10 ubuntu bash open-source
finished: true
---
...
개발을 진행하다 보면 때때로 Windows 환경을 지원하지 않는 경우가 종종 있습니다. 그렇기 때문에 우리는 가상머신 혹은 컨테이너나 Cygwin 같은 것을 이용해야 했습니다. 이런 문제점을 개선하기 위해 마이크로소프트와 우분투를 창립한 캐노니컬(Canonical)이라는 회사가 합작하여 **우분투 리눅스에서 수행되는 Bash Shell 바이너리 그 자체를 윈도우에서 수행할 수 있도록 만들었습니다.**

이 모든 것이 쉬워진 이유는 위에서 말했다시피 윈도우 10에서 Bash Shell 지원이 업데이트되었기 때문입니다. 이 기능을 `Windows Subsystem for Linux`라 하여 줄여서 `WSL`이라 부르고 있습니다.

이를 통하여 오픈 소스 기반의 개발 환경을 쉽게 구축할 수 있습니다.
...
> 아래의 구성은 IDE를 포함한 **고급 개발환경 구성입니다.** <br/>
...
5. vcxsrv 서버의 실행을 확인한 뒤 `xeyes` 명령을 치면 눈 모양의 위젯 확인
...
### 그 외
> Eclipse 같은 IDE는 이제 직접 Firefox로 웹에 접속하여 다운 받은 뒤 압축을 해제하여 설치할 수 있습니다.
