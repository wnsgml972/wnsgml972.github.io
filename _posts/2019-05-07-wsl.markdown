...
개발을 진행하다 보면 때때로 Windows 환경을 지원하지 않는 경우가 종종 있습니다. 그로인해 가상머신 혹은 컨테이너나 Cygwin 같은 것을 이용해야 했습니다. 
...
이 모든 것이 쉬워진 이유는 위에서 말했다시피 윈도우 10에서 Bash Shell 지원이 업데이트되었기 때문입니다. 이 기능을 `Windows Subsystem for Linux`라 하여 줄여서 `WSL`이라 부르고 있습니다. 이 기능을 사용하면 오픈 소스 기반의 개발 환경을 쉽게 구축할 수 있습니다.
