...
### 연속 메모리 할당
시간이 지나 컴퓨터의 구조가 발달되어 오면서 다중 프로그래밍 환경이 조성되었고, 여러 프로세스에서 한 메인 메모리에 대한 공간을 사용하게 되었습니다. 그러나 중간에 프로세스가 각각 끝나게 되면 사용하던 메모리는 빈 공간이 되므로 메모리 공간이 낭비되게 됩니다. 이것을 **외부 단변화**라고 합니다. 그리고 이것을 해결하고자 메모리 공간을 절약하기 위한 **연속 메모리 할당 방식**을 사용하게 되었습니다. 
